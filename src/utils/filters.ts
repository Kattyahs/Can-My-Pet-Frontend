...
export default function filterFood(
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<AnimalFoodPermission[]>(
          `${API_URL}/canmypeteatrelation/readByIdPet/${animalId}`
        );
  ...
  */
  ...
  return data.filter(
    (result) =>
      ...
      result.Food?.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
}
